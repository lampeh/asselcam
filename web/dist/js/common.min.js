 ...
 * Licensed under MIT (https://github.com/twbs/bootstrap/blob/master/LICENSE)
 * ======================================================================== */
+function(a){function b(b){var c,d=b.attr("data-target")||(c=b.attr("href"))&&c.replace(/.*(?=#[^\s]+$)/,"");// strip for ie7
return a(d)}
...
function(a){a.CSS||(a.CSS={});var b=a.CSS,c=function(a){this.message=a};c.prototype=new Error,c.prototype.name="InvalidCharacterError",b.escape||(
// http://dev.w3.org/csswg/cssom/#serialize-an-identifier
b.escape=function(a){for(var b,d=String(a),e=d.length,f=-1,g="",h=d.charCodeAt(0);++f<e;){
// Note: there’s no need to special-case astral symbols, surrogate
// pairs, or lone surrogates.
// If the character is NULL (U+0000), then throw an
// `InvalidCharacterError` exception and terminate these steps.
if(b=d.charCodeAt(f),0==b)throw new c("Invalid character: the input contains U+0000.");
...
1==f&&b>=48&&b<=57&&45==h?"\\"+b.toString(16)+" ":
// If the character is not handled by one of the above rules and is
// greater than or equal to U+0080, is `-` (U+002D) or `_` (U+005F), or
// is in one of the ranges [0-9] (U+0030 to U+0039), [A-Z] (U+0041 to
// U+005A), or [a-z] (U+0061 to U+007A), […]
b>=128||45==b||95==b||b>=48&&b<=57||b>=65&&b<=90||b>=97&&b<=122?d.charAt(f):"\\"+d.charAt(f)}return g})}("undefined"!=typeof global?global:this),
...
//
d.append('<li class="settings_info '+j.perma_orientation+'"><div class="settings_info_menu off perma_option_off"><a href="'+j.info_link+'"><span class="help_info icon"><span class="info">'+o.txt_help+"</span></span></a></div></li>"),
...
if(k&&F||m&&G||n&&H){for(
// Cookies abrufen
var I=document.cookie.split(";"),J="{",K=0;K<I.length;K+=1){var L=I[K].split("=");
// Spaces and Quotes getting removed
L[0]=a.trim(L[0].replace(/"/g,"")),L[1]=a.trim(L[1].replace(/"/g,"")),J+='"'+L[0]+'":"'+L[1]+'"',K<I.length-1&&(J+=",")}J+="}",J=jQuery.parseJSON(J);
// Container definieren
var M=a("li.settings_info",d);
// Klasse entfernen, die das i-Icon alleine formatiert, da Perma-Optionen eingeblendet werden
M.find(".settings_info_menu").removeClass("perma_option_off"),
// Perma-Optionen-Icon (.settings) und Formular (noch versteckt) einbinden
M.find(".settings_info_menu").append('<a href="#" class="settings">'+o.settings+"</a><form><fieldset><legend>"+o.settings_perma+"</legend></fieldset></form>");var N="r"+Math.floor(101*Math.random()),O=' checked="checked"';if(k&&F){var P="perma_on"===J.socialSharePrivacy_facebook?O:"";M.find("form fieldset").append('<input type="checkbox" name="perma_status_facebook" id="'+N+'_perma_status_facebook"'+P+' /><label for="'+N+'_perma_status_facebook">'+o.services.facebook.perma_display_name+"</label>")}if(m&&G){var Q="perma_on"===J.socialSharePrivacy_twitter?O:"";M.find("form fieldset").append('<input type="checkbox" name="perma_status_twitter" id="'+N+'_perma_status_twitter"'+Q+' /><label for="'+N+'_perma_status_twitter">'+o.services.twitter.perma_display_name+"</label>")}if(n&&H){var R="perma_on"===J.socialSharePrivacy_gplus?O:"";M.find("form fieldset").append('<input type="checkbox" name="perma_status_gplus" id="'+N+'_perma_status_gplus"'+R+' /><label for="'+N+'_perma_status_gplus">'+o.services.gplus.perma_display_name+"</label>")}
// Settings-Menue per Tastatur erreichbar machen, die Mouseevents werden getriggert
a(d).on("click","li.settings_info .settings",function(b){b.preventDefault(),"on"==a(this).data("keyb")?(a("li.settings_info",d).trigger("mouseleave"),a(this).data("keyb","off")):(a("li.settings_info .settings",d).trigger("mouseenter"),a(this).data("keyb","on"))}),
// Einstellungs-Menue bei mouseover ein-/ausblenden
a(d).on("mouseenter","li.settings_info .settings",function(){var b=window.setTimeout(function(){M.find(".settings_info_menu").removeClass("off").addClass("on")},500);a(this).data("timeout_id",b)}),a(d).on("mouseleave","li.settings_info",function(){var b=a(this).data("timeout_id");window.clearTimeout(b),M.find(".settings_info_menu").removeClass("on").addClass("off")}),
// Klick-Interaktion auf <input> um Dienste dauerhaft ein- oder auszuschalten (Cookie wird gesetzt oder geloescht)
a(d).on("click","li.settings_info fieldset input",function(b){var c=b.target.id,e=c.substr(c.lastIndexOf("_")+1,c.length),h="socialSharePrivacy_"+e;a("#"+b.target.id+":checked").length?(f(h,"perma_on",j.cookie_expires,j.cookie_path,j.cookie_domain),a("form fieldset label[for="+c+"]",d).addClass("checked")):(g(h,"perma_on",j.cookie_path,j.cookie_domain),a("form fieldset label[for="+c+"]",d).removeClass("checked"))}),
// Dienste automatisch einbinden, wenn entsprechendes Cookie vorhanden ist
k&&F&&"perma_on"===J.socialSharePrivacy_facebook&&a("li.facebook .switch",d).click(),m&&G&&"perma_on"===J.socialSharePrivacy_twitter&&a("li.twitter .switch",d).click(),n&&H&&"perma_on"===J.socialSharePrivacy_gplus&&a("li.gplus .switch",d).click()}})})}}}(jQuery),
// Source: src/js/misc/socialshareprivacy.js
jQuery(function(a){a("#socialshareprivacy").length>0&&a("#socialshareprivacy").socialSharePrivacy({css_path:"/socialshareprivacy/socialshareprivacy.min.css",lang_path:"/socialshareprivacy/lang/",services:{facebook:{perma_option:"off",dummy_img:"/socialshareprivacy/images/dummy_facebook.png",sharer:{status:"on",dummy_img:"/socialshareprivacy/images/dummy_facebook_share_de.png",img:"/socialshareprivacy/images/facebook_share_de.png"}},twitter:{perma_option:"off",dummy_img:"/socialshareprivacy/images/dummy_twitter.png"},gplus:{perma_option:"off",dummy_img:"/socialshareprivacy/images/dummy_gplus.png"}}})});